...
router.get("/allPacket", (req, res)=>{
  conn.query(
    "SELECT Packet.PID, Packet.deals_ID, Deals.concert_deal_ID, Concert_Deals.number_of_tickets, Concert_Deals.concert_deal_price, Deals.hotel_deal_ID, Hotel_Deals.number_of_rooms, Hotel_Deals.hotel_deal_price, Packet.s_deadline_package, Packet.deadline_package FROM Packet INNER JOIN Deals ON Deals.DID = Packet.deals_ID INNER JOIN Concert_Deals ON Concert_Deals.CDID = Deals.concert_deal_ID INNER JOIN Hotel_Deals ON Hotel_Deals.HDID = Deals.hotel_deal_ID",
    (err, result) =>{
      if (err) {
